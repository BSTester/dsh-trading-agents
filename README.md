...
| 键 | 默认 | 作用 |
|---|---|---|
| `sentiment_budget_seconds` | 600 | 一轮采集的**总预算**。耗尽即停止剩余标的（记 `skipped_symbols`），**退出 0** + warn 告警「情绪快照预算耗尽」——「今天没采完」而不是「失败」 |
| `sentiment_symbol_timeout_seconds` | 90 | **单个标的**的超时。超时只判该标的失败并继续下一个（不会一个卡住拖死整轮） |

两者之和有硬约束：`预算 + 3 × 单标的超时 < 900s`（作业上限），配置期就校验——超了会
fail-closed 报错并给出调整方向（默认 600 + 270 = 870 ✓）。采集是**攒历史**，允许采不完，
