...
class SnapshotTest(StoreAccessBase):
    ...
    def test_endpoints_manifest_is_58_and_cached(self):
        endpoints = sa.endpoints()
        # 58 项 = 22 项基础清单（WP7 面板退役后冻结在 sa._BASE_ENDPOINTS，原序=已删
        ...
        # 3 端点（openapi_config/openapi_test/openapi_oauth）。整表钉死见
        # test_wp6_tables_lock。
        ...
        self.assertEqual(endpoints[-36], "factors-history")
        self.assertEqual(endpoints[-35:-29],
                         ["trade_place", "trade_modify", "trade_cancel",
                          "account_positions", "account_orders", "account_funds"])
        self.assertEqual(endpoints[-29:-21], list(sa.FUTU_ENDPOINTS))
        self.assertEqual(endpoints[-21:-12], list(sa.WP8_MARKET_ENDPOINTS))
        self.assertEqual(endpoints[-12:-6], list(sa.WP8_TRADE_ENDPOINTS))
        self.assertEqual(endpoints[-6:-3], list(sa.WP8_PUSH_ENDPOINTS))
        self.assertEqual(endpoints[-3:], list(sa.WP8_SETTINGS_ENDPOINTS))
        # 业务确认两端点必须在白名单里（HTTP 面据此注册路由）
        self.assertEqual(endpoints[-38:-36], ["confirmation", "confirm-decide"])
