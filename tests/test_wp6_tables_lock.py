...
覆盖：
  * ``store_access.endpoints()`` ≡ 55 项内嵌清单（22 项基础清单按已删 endpoints.js
    原序冻结 + 7 项 WP7 服务自有端点 + 8 项 WP8 富途实时直通端点 + 9 项 WP8 OpenAPI
    行情端点 + 6 项 WP8 OpenAPI 交易只读端点 + 3 项 WP8 推送订阅管理端点，逐项与顺序都
    钉死）；
...
class EndpointListLockTests(unittest.TestCase):
    def test_endpoints_is_frozen_55_item_list(self):
        """``store_access.endpoints()`` ≡ 22 基础 + 7 WP7 + 8 WP8 直通 + 9 WP8 行情
        + 6 WP8 交易 + 3 WP8 推送 = 55 项，同序。"""
        self.assertEqual(store_access.endpoints(),
                         ...
                         + WP8_PUSH_ENDPOINTS)
        self.assertEqual(len(store_access.endpoints()), 55)
        ...
        self.assertEqual(store_access.endpoints()[-35:-33],
                         ["confirmation", "confirm-decide"])
        self.assertEqual(store_access.endpoints()[-33:-26], WP7_ENDPOINTS)
        self.assertEqual(store_access.endpoints()[-26:-18], FUTU_ENDPOINTS)
        self.assertEqual(store_access.endpoints()[-18:-9], WP8_MARKET_ENDPOINTS)
        self.assertEqual(store_access.endpoints()[-9:-3], WP8_TRADE_ENDPOINTS)
        self.assertEqual(store_access.endpoints()[-3:], WP8_PUSH_ENDPOINTS)
        self.assertEqual(store_access.endpoints()[0], "snapshot")
        # 无重复；重复调用返回等值副本（调用方改动不污染后续结果）
        self.assertEqual(len(set(store_access.endpoints())), 55)
