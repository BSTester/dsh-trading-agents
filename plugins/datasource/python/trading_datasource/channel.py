...
def call_openapi(client, method, params, timeout=None):
    """按 ``"组.方法"`` 点号路径调用（如 ``"f10.statements"``、``"basic.rehab"``）。

    未知组/未知方法 → ``ValueError``（本地拒绝，零网络往返）——与传输层
    「签名即白名单」同一口径：调用点写错方法名要立刻炸，不能悄悄走到别的方法上。

    ``timeout``（秒）**兑现到本次调用链的传输超时**：经 ``client.timeout_scope``
    包裹（WP13 审查 M2——sim 写路径的 timeout 过去在 REST 分支被静默丢弃，而
    「传输失败 → unknown」的判定正建立在该时限上）。注入替身若不支持 scope
    （无 ``timeout_scope``）则忽略该参数——4 参老替身零改动。
    """
    ...
    if timeout is None or not hasattr(client, "timeout_scope"):
...
# ---------------------------------------------------------------------------
# 模拟交易通道适配器已拆到 ``channel_sim``（WP13 审查次要项：本模块聚焦「通道选择 + 取数」）
# ---------------------------------------------------------------------------
# 公开名在此**再导出**，调用方（``trading_core.broker`` / 平台闸门 / 测试）零改动：
#   channel.sim_call / channel.SIM_TOOL_ROUTES / channel.to_micros / channel.to_micros_end
# 适配器的翻译表与实测差异见 ``channel_sim`` 模块 docstring。
from .channel_sim import (  # noqa: E402  （末尾再导出：避免调试期未用名告警）
    SIM_TOOL_ROUTES, sim_call, to_micros, to_micros_end)
