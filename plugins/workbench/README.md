...
见仓库根 README）；legacy 面板（client.js）与 Host Connection RPC 已于 2026-09-16
退役删除。本插件只剩 Host 半边：`tradingWorkbench` 服务保存研报、量化预览和
Harness 已观察到的富途账户响应，是 engine 对话工具（研报/预览/取消）与账户策略
（模式互斥、调用租约、观察记录）的进程内依赖。
