...
本轮落地（真机证据见 `docs/v3-round4-summary.md` 与 `docs/e2e-and-data-gaps.md` §25）：
事件驱动策略、统计套利策略、NLP 事件识别（33 类）、成长因子补齐（0→4/5）、
roe/roa 接入 SQLite 读取（0→3/5）、价量 `factors.as_of` 全链开放（HTTP/MCP→compute→CLI，
缓存键含完整 payload）、以及三处**研究正确性修复**（历史矩阵 z-score 与 ticker 错配、
事件收益重复累计、stat-arb 把 DF 启发式谎称"协整"→改名 `screenPassed`）。

**第四轮计数（31 条）**：**对齐 16 / 部分 13 / 缺失 0 / 不适用 1 / 未验证 1**
（第三轮 7/18/0/1/1 的口径经逐条重校准——部分"对齐"判定按证据从严复核实为部分）。
- 完全对齐率 = 16/30 可评 ≈ **53%**；覆盖（有实现）= 30/30 = **100%**；对齐+部分 = 97%。
- 本轮状态变化：FR-DATA-003 部分→对齐（价量 as_of 开放）；FR-STRAT-002 的缺失子项
  （事件驱动/统计套利）→ 已实现，但"数千次回测"**仍未达标**（上限 64 格/次）→ 整条维持部分；
  FR-STRAT-003 事件识别落地 → 整条维持部分（多源融合弱、情绪列读按日快照）。
- 仍为"部分"的 13 条与限制：GATEWAY-001（outbound 豁免后仅剩命名口径）、TOOLS-001/002
  （方法名、等长 null/render/isConcurrencySafe/skill）、DATA-001（A 股 -9、live 未实测）、
  DATA-002（Tushare 未配凭据）、STRAT-001（情绪 0/5、另类显式取数）、STRAT-002（数千次）、
  STRAT-003（多源融合）、EXEC-001（live 未实测）、EXEC-003（margin_debt/byFactor 上游缺）、
  MON-002（TradingView、会话状态数据待积累）、§4.2（RBAC 0、私钥明文 PKCS8+0600）、§8.2（2/9）。
- 未验证 1：NFR §4.1——已有口径的两项**实测超标**（headless 单次 33-44s > 30s；
  MCP 均值 2.38s > 2s）；行情 <100ms 与可用性 99.9% 仍无埋点/长时序。
