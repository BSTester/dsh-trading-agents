...
| 授权续期 | 尚无插件级自动续期。**实测：access token 过期时，`initialize` 仍然成功，而所有 `tools/call` 返回 JSON-RPC `internal error`**（不是 401）——所以 `internal error` 确实是 token 过期的一种表现，不能一概归因于网络。渠道状态页已改为真调一次业务工具来判定，并提示先续期 |
