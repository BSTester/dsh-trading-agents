...
| 授权续期 | **官方 `expires_in = 7200`（2 小时）**，续期不换发 refresh_token。实测 token 过期时 `initialize` 仍成功、而所有 `tools/call` 返回 `internal error`（不是 401），因此旧探测长期误报「token 有效」。现已：① 探测真调一次业务工具；② 遇该特征自动续期并重试一次；③ 状态页显示剩余有效期并在不足 10 分钟时预警。仍需注意：`internal error` 无法区分凭证过期与服务端故障，自动续期失败时应重试而非断言故障原因 |
