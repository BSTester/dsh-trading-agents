...
> **本文件是 2026-09-18 从仓库根 `README.md` 拆出的「功能说明与操作细节」。** README 现在
> 只讲项目介绍与安装方法（一键安装 / 装完之后 / 富途授权 / 更新到最新版本），其余内容**逐字**
> 迁到这里，信息零丢失；章节标题沿用拆出前的原文，文件内相对路径已按 `docs/` 层级修正，
> 原来自指 README 的「见上 / 上节」也改成指向新位置。
>
> 导航：[README](../README.md) · [架构与交互边界](architecture.md) · [运维 Runbook](RUNBOOK.md) ·
> [工具与数据源限制](TOOL-LIMITS.md) · [E2E 验收](E2E-ACCEPTANCE.md) ·
> [交接与复审](HANDOVER.md) · [实盘边界](P4-live-trading.md) ·
> [工作台方案](QUANT-WORKBENCH-PLAN.md) · [安装协调员手册](../install/HARNESS_SETUP.md)。
...
| `~/.dsh/trading-venv/bin/python -m trading_core watchlist-init --from-index SH.000300 [--limit N] [--force]` | 首启配置关注池（见仓库根 README「装完之后」）；已有非空关注池时默认拒绝覆盖，`--force` 才覆盖 |
...
- **二、常驻纪律**：直接提交 `main`（本仓库惯例）；每次提交保持三套测试全绿（命令见仓库根 `AGENTS.md`「二、常驻纪律」）；
...
> [RUNBOOK.md](RUNBOOK.md)「平台服务」一节。**前台跑法
...
- **推送订阅管理面（任务 6）**：`push_status`（与 `/healthz` 的 push 同形，前端页头 10 秒
  ...
  受限字段与通道边界见 [TOOL-LIMITS.md](TOOL-LIMITS.md) 第八节。
...
**改配置的正确入口是 Web 设置页**（`#/settings` → 「自动流水线」卡片）：策略、关注池键、
各市场执行时刻、晚间对账时刻都是**选择器**（WP22，2026-09-18），下拉里的中文说明写的是
「这个策略怎么选股」；保存时服务端**校验策略名**，非法值直接拒绝并在错误信息里列出全部
合法取值（见下）。也可以直接改 `trading-platform.json`：
...
`strategy` 的合法取值只有两类（`settings_api.save_auto_pipeline` 写入侧 fail-closed 校验；
**打错一个字母不会再静默保存**——旧实现在这种情况下每天软跳过「策略未注册」，页面上只是
「没动静」）：

| 取值 | 中文口径 | 自动流水线 |
|---|---|---|
| `watchlist_rsi` | 关注池 RSI 组合（池内超卖等权，按风控上限截断） | 可用 |
| `momentum_value_top5` | 动量+价值前 5（沪深 300 成分内多因子打分取前 5） | 可用 |
| `ma_cross` | 双均线（单标的） | **不可用**：单标的策略没有 `target_weights`，选中它当日 `build_plan` 作业失败（设置页里该项可见但禁止选择） |
| `rsi` | RSI（单标的） | 同上 |
| 研究页**已批准**（`status=enabled`）的 `rule_id` | 规则的下拉标签是「rule_id（已批准规则）——假设摘要」 | 可用；`candidate/validating/passed/failed/disabled` 的规则**不可消费**（`planner._resolve_strategy` 每次回查 DB 状态） |
...
未决项见 `TOOL-LIMITS.md`「模拟盘读能力与自动执行下单账户」。
...
排障与演练（假时钟、窗口超时、熔断拦截）见 [RUNBOOK.md](RUNBOOK.md)「自动流水线（WP9）」。
...
排障口径见 [RUNBOOK.md](RUNBOOK.md)「目标外持仓清出没生效 / 清了但买不进」。
...
排障见 [RUNBOOK.md](RUNBOOK.md)「人工下单时段闸门（WP19）」。
...
[RUNBOOK.md](RUNBOOK.md)「值班研究员（L3）」。

## 工作台与指令示例

按上文「服务不是可选项：独立工作台服务」启动独立 Web 后在浏览器打开 `http://127.0.0.1:8397`（Harness 内没有工作台面板，
...
**按市场查看（WP23，2026-09-18）**：页头右上角有「市场筛选」选择器
（全部市场 / A股 SH / 港股 HK / 美股 US，选择记在浏览器 localStorage）。它只做
**客户端展示层过滤**——不改任何请求参数，也不改任何账户配置：

| 页面 | 选择器的作用 |
|---|---|
| 概览 / 组合 / 风险 / 执行 / 审计 / 因子 / 信号 | 表格与卡片只列该市场的行；「全部市场」下这些表会**多一列「市场」**（如「A股 SH」），跨市场混排时一眼能分辨 |
| 计划 | 只影响「计划列表」的展示；**执行入口恒指向最新计划**，不随筛选改变（提交的是它的内容哈希） |
| 流程 | 只渲染该市场的作业链卡片；「全局（晚间链）」与 kill/halt 状态仍显示 |
| 调度 | 作业表只列该市场作业 + `GLOBAL:*` 作业；daemon 状态、心跳与告警仍显示 |
| 行情 / 事件 / 资金 | **不按市场过滤**（一次只查一个标的，过滤会与输入冲突）；标的与筛选不一致时给一句提示 |

筛选后为空时页面会写明原因（例如「当前筛选：港股 HK —— 本页无港股数据（共 37 行，均属
其他市场）。」），不会只显示「暂无数据」；标的没有交易所前缀、市场无法判定的行（如券商
回报里的裸代码 `00981`）**不猜市场**，如实标注并在被排除时报数。

> 富途各工具的**已知限制与规避方式**（哪些工具会静默返回空、A 股缺哪些实时权限、
> 哪些工具是上下文炸弹）见 [TOOL-LIMITS.md](TOOL-LIMITS.md)，
> 由一次 99 个工具的全量实调体检得出。
...
渠道不会整体不可用。详见 [QUANT-WORKBENCH-PLAN.md](QUANT-WORKBENCH-PLAN.md)。
...
## 获取富途 token 的细节（有效期 / 授权范围实测 / 技术细节）

> 「没有 token 也能用」与那条授权命令的摘要留在仓库根 README 的「富途授权」一节；本节是完整细节。
...
独立 Web 的确认卡片作答，见 [architecture.md](architecture.md) 的「业务确认表述统一」）**。
...
监控熔断都已完成。实盘前置条件与边界见 [P4-live-trading.md](P4-live-trading.md)。
