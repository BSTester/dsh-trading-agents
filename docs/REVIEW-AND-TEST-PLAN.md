...
- **质量因子**：新增 `quality.py` + `quality` 接口，从 `quote_financials_statements`
  ...
  **ROE/ROA 明确列为不可得**：实测富途 91 个工具里没有任何资产负债表接口，
  所有报表类型返回的都是利润表。
...
| 质量因子（财报计算） | ✅ 实测 | `US.AAPL`：毛利率 50.06% / 营业利润率 32.62% / 净利率 27.23% |
...
| 质量因子 | 毛利率/营业利润率/净利率/研发占比/实际税率/同比 | ✅ 已做；**ROE/ROA 不可得**——富途 91 个工具无资产负债表接口，已列入 `unavailable` 而非估算 |
