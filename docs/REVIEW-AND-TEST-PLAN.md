...
**共同的限制**：Authorization 头在**会话组合挂载时求值一次**，preset 目录没有 watcher，
所以续期**只对新建会话生效**。已挂载的会话必须新建会话才会重新读取。
...
| 长会话 2 小时悬崖 | 保活只对**新建会话**生效 | `agentPresets.recompose` 未验证（`swap` 明确因 session 已开始而 locked） |
| 权益曲线 | `analytics equity` 返回"无可用于盯市的日线数据"（本地台账为空） | 未做每日盯市持久化 |
| 质量因子 | ROE / 毛利率等未接入 | 未做 |
