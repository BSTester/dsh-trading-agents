...
**半日市纳入「会话已收盘」判定（WP19 修订，2026-09-18）**：上一条的会话收盘用的是常量
全天界（`SESSION_CLOSE_BEIJING`：SH/HK 当日 15:00/16:00、US 北京次日 05:00），而日历行里
的 `trade_second`（实测 SH 14400 / HK 19800 / US 23400）在生产代码里**从未被使用**——
半日市因此没有建模：港股半日市真实收盘在本地 12:00，旧口径要到北京 16:00 才判「已收盘」，
当天整天不产出计划；美股半日（13:00 ET）同理。修法：

- 判定顺序改为「**先解析会话本地日（回落最近交易日）→ 再用该日真实收盘判是否已收盘**」
  （原先先判收盘、再解析会话日）；未收盘 → `None`（调用方软跳过，语义不变）；
- **交易日**先解析——收盘判定用**该日**的 `trade_second`，不是「北京日」的；
- 收盘口径：`trade_second` 缺失或等于全天秒数 → **逐字沿用** `SESSION_CLOSE_BEIJING`
  （美股 EST 05:00 的偏保守 DST 口径**刻意不动**：换成按 zoneinfo 精算的真实收盘会让 EDT
  期间 04:00–05:00 的补跑提前放行）；**小于**全天秒数（半日/提前收盘）→ 收盘 =
  **开盘 + `trade_second`**，按**单段（不含午休）**算：港股半日 09:30+2.5h = 12:00、
  美股半日 09:30+3.5h = 13:00。用「全天收盘 − 缺口秒数」会把港股算成 13:00（错），
  因为半日市在两地实践里都**只有上午**、午休不适用；
- 统一实现是 `trading_core/sessions.py`（时区用 `zoneinfo`），日历行经只读助手
  `store.calendar_row` 取（planner 不写裸 SQL）。同一模块还提供**可委托窗口**，供人工
  下单时段闸门使用（见 §4.3 末尾「两条路径的时段分工」）。
...
**两条路径的时段分工（WP19 补记，2026-09-18）**：本作业（自动）与工作台
`plan-execute` 端点（人工触发）**写的是同一种指令文件**，但时段约束不同源，刻意如此：

- **自动**：守卫 9 的**执行窗口**（`exec_at` + `exec_window_minutes`）——它管的是「补跑
  不要发生在错误时点」，与市场是否闭市无关（`exec_at` 本身就配在开盘后，如 SH 09:35）；
- **人工**：`platform/server/trading.py` 的**时段闸门**（`trading_core.sessions` 的市场
  可委托窗口）——它管的是「人现在点的这一下，市场是不是明确闭市」。该闸门在
  `TradeGate._write`（`trade_place`/`trade_modify`，撤单放行）与 app 的 `plan-execute`
  入队处生效，**不覆盖本作业与指令轮询**（`handle_command`/`execute.run` 本轮明确未动）。

因此改自动执行的时段行为请改守卫 9；改人工下单的时段行为请改 `sessions` 的窗口表。
两者混用会得到「自动窗口」与「人工闸门」互相打架的口径。
