...
兜底路径：Web 会话打开或恢复时，Harness 同样读该队列补跑积压任务——与定时器路径
共用同一状态机，靠状态迁移互斥，不会重复执行。
...
- 由 L1 机械层入队（新作业 `enqueue_research`，紧随 digest 之后），**入队是机械动作、
  零 LLM**。
...
|---|---|---|---|
| `daily_brief` | 每交易日数据就绪后 | as_of、market、digest 引用、sentiment/f10 快照引用 | 资讯影响简报 → 研究页 |
| `factor_patrol` | 每交易日 | as_of、因子清单、IC 历史窗口 | 因子衰减巡检报告 + 异常告警 |
| `mining_round` | 每周一次 | as_of、候选因子域、数据引用 | 因子/规则候选提案 → 候选池（待人工批准） |
...
- **A（主）**：外部定时器每交易日唤醒一次（默认「数据就绪后 30 分钟」，可配置）；
  唤醒后 headless 会话循环领取至队列清空，单次运行设总时长上限（防失控），
  超限即退出并把未完成任务留在队列；
- **C（兜底）**：定时器没跑成（机器关机、异常退出、未装定时器）时，你打开/恢复会话
  即补跑积压——**任务不丢、只延迟**；
