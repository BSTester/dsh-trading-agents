...
## 执行期追加任务登记（WP9 实作期间新增，2026-09-16/17）

计划任务 1–8 之外的追加项，均为执行中暴露的真实缺口（详细背景见规格 §4.2/§4.3/§4.4 的「实现期补记」与 §十二 风险清单）：

| 追加项 | 触发原因 | 提交 |
|---|---|---|
| 冻结即登记 OMS 订单 | `execute.run` 取不到 planner 生成的单（既有接线漏洞，人工与自动两条执行路径都受影响） | `80ec1b8` |
| live 计划自动生成 | 用户要求「只有实盘执行时才需人工」→ live 计划也应生成；权益字段经官方 `get-funds` 文档核实为 `total_assets` | `ec140d6` |
| 第九守卫（执行窗口） | 调度器 tick-first 补跑会在错误时点执行（收盘后被规则 3 拒单并消耗计划） | `d169d26` |
| 对账先同步券商事实 | sim 无 WS 成交事件 → 本地台账不学习 → 系统性假差异 → 每日 halt 自锁熔断 | `853143c`（fills 回填）、`05448c1`（状态按累计量推进） |
| `DSH_FAKE_NOW` 跨进程假时钟 | 作业经 CLI 子进程执行，`tick(now=…)` 传不进子进程 | `50fbc99` |
| 风险预算定量 + 策略上限 | 权重全额定量与规则 4/5 结构性冲突 → 自动流水线无法成交 | `a092ed8` |
| 受管集合 + 执行 ctx 券商持仓 | 只买不退（无退出路径）；规则 5 对 SELL 判成新建仓位而拒 | `61663c8` |
| 美股日期空间修正 | 就绪门按北京日期判定 → 美股腿静默永不生成 | `3950e80` |
| K1/I1–I4 质量修复 | 代码质量审查：策略无市场维度（美股饿死）、`watchlist` 键形同虚设、过期计划孤儿订单、halt 可见性、关注池三份实现 | `5c60cf7` |
| S1 daemon 拆分 | 750+ 行 6 类职责，后续 WP 还要加三个作业 | `8df9813` |

**接受的技术债（登记备查）**：

- 规则 5 的卖出方向语义由 `execute._order_ctx` 在调用侧折算（更干净的长期修法是让规则 5 方向感知，需解冻 `risk.py`）；
- `positions_count` 只统计计划所属市场，跨市场叠加会低估规则 6；
- `daily:digest` 的 `untracked` 只有计数（`snapshot-reconcile` 有明细）；
- live 版 `$DSH_HOME/trading-python` 安装副本刷新与作业子进程解析路径，留待全局验收统一处理。
