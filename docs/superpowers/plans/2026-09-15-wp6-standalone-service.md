...
**文件：** 创建 `platform/web/package.json`、`platform/web/vite.config.mjs`、`platform/web/index.html`、`platform/web/src/main.jsx`、`platform/web/src/app.jsx`、`platform/js/services/api.js`、`platform/js/services/hooks.js`、`platform/web/src/pages/`（11 个占位页组件，任务 9–11 逐个替换为真实现）
...
- [ ] **步骤 4：`platform/js/services/api.js`（数据层，规格 §4.3：两层缓存变一层半）**
...
- [ ] **步骤 5：`platform/js/services/hooks.js`**
...
**文件：** 创建 `platform/js/charts/geometry.js`、`platform/web/tests/geometry.test.mjs`、`platform/js/charts/line.jsx`、`platform/js/charts/kline.jsx`、`platform/js/charts/heatmap.jsx`
...
要点：错误文案与 `platform/js/services/mode.js` 的本地预检文案**逐字相同**；
...
13. **模式切换 UI 漏配任务（最终审查发现的阻塞项 A，已修复）**：成因是**初版计划的任务 8–12
    ...
    本次补齐：新增 `platform/js/services/mode.js`（纯函数 `switchModeRequest` /
...
14. **前端 `snapshot.endpoints` 预检初版未实现（已补齐）**：规格 §4.3 要求「未声明端点直接
    ...
    `platform/js/services/endpoints.js`（`declaredEndpoints` 非数组→null；
