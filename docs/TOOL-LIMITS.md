...
### 模拟交易订单状态码：官方**逐值发布**（2026-09-17 核对）

`sim-trade/order-list.md`（今日订单）的字段说明原文：

```
status  int  2=已提交 3=部分成交 4=全部成交 5=已撤 6=拒绝
qty / cum_qty / price / avg_fill_price 均为**字符串**
```

含义与影响：

- 本仓库早先「状态码枚举未公开、禁止解释整数」的说法**对模拟交易不再成立**——这是官方
  发布的契约，解释它是读契约而不是猜标签。对账（`reconcile`）因此在同步券商事实时，
  除按累计成交量推进外，还会对**表内**终态码做终态对齐（`5` 且无成交 → `cancelled`；
  `5` 且有部分成交 → 按**数量口径**给 `partial`，绝不把有成交的单标成撤单；`6` → `rejected`；
  `4` 但累计成交不足委托量 → 视为矛盾、不迁移）。**表外**的码仍然不解释（`SIM_ORDER_STATUS`）。
- 托管 MCP 通道返回的**同一族**整数码（`sim_trade_history_order_list` 的 `status=5`）经实测
  与官方文档一致，故同一张表适用；其他上游（如行情类）的整数码仍按未公开处理。
