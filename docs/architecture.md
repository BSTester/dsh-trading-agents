...
> `install/research-duty.{service,timer}`，唤醒 19:20）与 L3 端到端验收（任务 5）已落地。
> L3 的两条消费路径共用同一队列：定时唤醒是**自动**路径，会话侧补跑是**由 `AGENTS.md`
> 启动纪律驱动的首次交互动作**（非后台自动；无 turn 不消费）。定时器没跑只延后不丢任务；
...
> 状态：**三层全部已实现**（WP9–WP15）。L3 的消费路径有两条，**自动性不同，不得混说**：
>
> 1. **主路径（自动）**：外部定时器（`install/research-duty.{service,timer}` →
>    `scripts/research_duty.sh` → `dsh --profile headless`）按 `OnCalendar=Mon..Fri 19:20`
>    唤醒 headless 会话消费队列；
> 2. **兜底路径（非自动）**：会话开始/恢复后的**首次交互**先消费积压——由仓库根
>    `AGENTS.md` 的**启动纪律**驱动（首个 turn 用 `research_tasks_claim` 探测队列，
>    有任务先按 `research-institute` 技能的值班模式处理再回应用户）。**没有任何 turn 的
>    会话不会自动发生任何事**；任务不丢、只延迟。
>
> 两条路径消费同一张 `research_tasks` 表，靠状态迁移互斥（超时 `running` 在下次领取时被
> `reclaim`）。定时器没装/机器关机/headless 被杀都只**延后**，不丢任务。

平台的任务按「谁触发、谁执行、有没有 LLM」分三层，**层间只经文件/API 交换数据，不相互调用**：

| 层 | 触发源 | 执行体 | LLM | 产物 |
|---|---|---|---|---|
| **L1 常驻机械调度** | 交易日历 + 60 秒 tick | 服务内调度器（`server/scheduler.py` + `trading_core.autopipeline`/`autopilot`/`planner.plan_auto`） | ❌ | 数据、因子/情绪快照、计划、订单、对账、摘要（`digest`） |
| **L2 Harness 即时任务** | **人在对话里发起** | Harness 主会话 + 子代理（`research-institute` 技能） | ✅ | 研报（`research_publish`）、规则提案（`rules` 表候选池） |
| **L3 定时 Harness 任务**（值班研究员，WP15） | 外部定时器 + `dsh --profile headless` | Harness headless 单次运行，消费 `research_tasks` 队列 | ✅ | 每日资讯简报、因子巡检、挖掘轮提案 |

三条不可越界（实现期回归清单）：

1. **L1 永不调 LLM**——信号侧必须可回测、可复现；`factors.py` 的「情绪永不入因子计算」
   是同一原则的另一面（资讯/情绪先按 PIT 攒历史，满 250 交易日才可申请走验证门）。
2. **L2/L3 永不直接下单、永不直接启用策略**——研究产出必须过机械验证门
   （`rules-validate`：IC t 检验 + 分层单调）**且**由人在 Web 批准（`rules-decide`）
   才成为规则；批准后仍需经 `auto_pipeline` 配置引用，执行永远过风控 8 规则唯一入口。
3. **服务进程永不承载 LLM 循环**——L3 由外部定时器拉起 DSH 自身
   （`dsh --profile headless`）完成，服务只提供队列读写端点。

## 组件职责

| 组件 | 责任 |
|---|---|
| 根目录 preset | 新建「交易智囊模式」，组合 persona、原生工具、skill 和 MCP；persona 按三条路径路由（快路径 `quant-trading` / 深度研报 `trading-agents` / 生产线 `research-institute`） |
| `skills/trading-agents/SKILL.md` | Harness 主会话与子代理执行 12 角色、6 阶段研究，数据不足显式说明 |
| `skills/research-institute/SKILL.md` | **研究院生产线**（WP14）：四子代理分工（采集/假设/检验/研报）+ 工具通道清单 + 产出协议（rules JSON）+ 八条硬规则（不写代码进核心库、验证不过不调阈值重跑、禁用端点清单、社媒三要素、未成熟因子不得进 `factors`、不得直接下单、数据必须来自工具、人工批准是唯一上岗通道） |
...
| `scripts/research_duty.sh` + `install/research-duty.{service,timer}` | **L3 值班研究员的唤醒入口**（WP15）：探活平台服务（不可达即非零退出并给指引，**不自己拉起服务**）→ 拼装提示词（内含领取→处理→回报循环与禁用写端点点名清单，防定时执行体提示漂移）→ `dsh --profile headless "<任务>"` 单次运行（`timeout` 包裹，默认 1800s）→ 日志落 `~/.dsh/logs/research-duty-*.log`。systemd timer `Mon..Fri 19:20`（= `reconcile` 19:00 → digest → `enqueue_research` 19:05 之后再唤醒）+ `Persistent=true`（错过的触发在恢复后补跑）；service 不带 `[Install]`——只有 timer 能拉起它，避免每次登录跑一次值班。两条消费路径（本脚本自动 / 会话首次交互的启动纪律兜底，见 `AGENTS.md`）共用 `research_tasks` 表 |
...
| `research-tasks-claim`（WP15，**进程内动作端点**，TTL 0） | `{}`（**空载荷**：领取=取队首，任何参数只可能被用来越权指定任务） | L3 值班研究员领取一条任务：先回收超时 running（`research_queue.reclaim`，幂等且便宜；**超时累加 `timeouts`、不消耗 `attempts`**——领取未回报≠尝试失败，R2 2026-09-16 审查），再对队首载荷做**领取侧二次校验**（队列即攻击面：直改库塞进来的自由文本键在领取这一刻被拒——任务**不被领走**、状态保持 pending、留 **critical 告警**等人介入，拒绝而非猜测执行）；通过后 `store.claim_task` 置 running 并返回 `{task: {task_id, kind, as_of, market, payload, status, attempts, timeouts, …}, reclaimed: {requeued, failed, kept}}`；空队列 → `{task: null}`。**进 MCP 工具面**（`research_tasks_claim`） |
| `research-tasks-report`（WP15，**进程内动作端点**，TTL 0） | `{task_id*, ok*: bool, result_ref?, err?}`（字段白名单只有结果三件套——**没有改任务载荷的位置**；`ok` 必须是真布尔，真值判断会把 `"false"` 记成 done） | L3 回报结果：`ok=true` → `done` + `result_ref`；`ok=false` → `attempts+1`，< 3 次回 pending（保留 err 供下次领取者判断，重试不等于盲试）、达 3 次转 `failed` + **warn 告警**（`research_queue.report` 发，detail 同时带 `attempts=`/`timeouts=`）。**终态幂等**（R1 2026-09-16 审查）：`done`/`failed` 再回报原样返回、状态与计数不变、**不重复发告警**——工具重试与网络重放不会把已完成的任务拉回队列。未知 task_id / 非法参数 → `trading/invalid-operation`。**进 MCP 工具面**（`research_tasks_report`） |
...
**为什么分三档**：数据面一次补入 16 个端点，若全部 1:1 进模型工具面会从 59 涨到 75（16 项直通），
