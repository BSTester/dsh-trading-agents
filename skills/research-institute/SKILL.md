...
| **值班模式（L3）** | 外部定时器唤醒 headless 会话，或用户打开会话时补跑 | **本节**：消费队列里已排好的任务 |
...
|---|---|---|---|
| `daily_brief` | `as_of`、`market`、`digest_ref`（当日口径摘要）、`symbols`（关注池）、`refs`（当日各快照表的行数） | 读 `digest_ref` 指向的当日摘要与 `refs` 里的情绪/F10/做空/板块快照，产出**资讯影响简报**：逐条带来源与时间，区分「数据事实」与「叙事」；空数据如实写「无数据」，不编 | 简报（研究页可见；`research_publish` 发布则带 `sources`） |
...
| `mining_round` | `as_of`、`refs` | 跑一轮因子/规则挖掘（用上文的四子代理分工与标准流程），产出候选提案；提案先 `rules-validate` 自检 | rules JSON 提案（**候选池，待人批准**）+ 研报 |
