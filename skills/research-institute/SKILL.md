...
1. **领取即探测**：按 `AGENTS.md` 的启动纪律，调 `research_tasks_claim`（无参数）——
   用它的**返回值**判断队列，而不是先去看清单；
2. 返回 `task` 非空 → **先进入本值班模式消费积压**（循环见下节），再处理用户请求；
3. 返回 `task = null` → 队列为空，直接处理用户请求（不空转、不为此打断用户）；
4. 需要给人看的队列清单时，用 Web 的 `research-tasks-list` 端点——**它不是执行体的
   工具，工具面里没有它**；执行体只需要「领取」。
...
**领取必须闭环（2026-09-16 审查 R2）**：领到的每一条都要回报收尾。处理不完、中途被打断、
发现这条做不了——都要调 `research_tasks_report(ok=false, err="...")` **把话说清楚再走**。
领取而不回报的任务会停在 `running`，要等 30 分钟超时回收；连续三次超时会被判 failed，
而它其实**一次都没被真正尝试过**。被打断可以，静默离开不行。

`research_tasks_report(task_id, ok, ...)`：成功传 `ok=true` 与 `result_ref`（产出引用）；
失败传 `ok=false` 与 `err`（写清原因，下一次领取者要靠它判断是不是盲试）。**真失败**达
3 次（`attempts`）任务转 failed 并发告警；**超时未回报**走独立的 `timeouts` 计数，
达 3 次才 failed，err 写「执行体未回报」——两种失败在告警里用 `attempts=`/`timeouts=`
两个计数区分。终态任务重复回报是**幂等**的（原样返回、不重复告警），所以重试回报不必
先查状态。**不要**绕过队列直接重跑。
